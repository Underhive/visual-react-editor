...
export function Selectable(visbug) {
  const page              = document.body as any
  let selected            = [] as any
  let selectedCallbacks   = [] as any
  let labels              = [] as any
  let handles             = [] as any
  ...
  const listen = () => {
    ...
    // hotkeys(`${metaKey}+alt+c`, on_copy_styles) // TODO: add edit here ucc
    // hotkeys(`${metaKey}+alt+v`, e => on_paste_styles()) // TODO: add edit here ucc
    hotkeys('esc', on_esc)
  ...
  const on_copy = async e => {
    ...
    if(window) {
      ...
      if (selected[0] && globalThis.node_clipboard !== selected[0]) {
        e.preventDefault()
        ...
        const { state } = await (navigator.permissions as any).query({ name: 'clipboard-write' })
  ...
  const on_cut = e => {
    let fakeWindow = window as any
    if (selected[0] && fakeWindow.node_clipboard !== selected[0]) {
      let $node = selected[0].cloneNode(true)
      $node.removeAttribute('data-selected')
      fakeWindow.copy_backup = $node.outerHTML
      e.clipboardData.setData('text/html', fakeWindow.copy_backup)
      const el = selected[0]
  ...
  const on_paste = async (e, index = 0) => {
    ...
    const potentialHTML = clipData || globalClipboard || fakeWindow.copy_backup
  ...
  const on_copy_styles = async e => {
    ...
    globalThis.copied_styles = selected.map(el =>
      getStyles(el))

    try {
      const colormode = $('vis-bug')[0].colorMode

      const styles = globalThis.copied_styles[0]
        .map(({prop,value}) => {
  ...
  const on_paste_styles = async (e, index = 0) => {
    if (globalThis.copied_styles) {
      selected.forEach(el => {
        globalThis.copied_styles[index]
          .map(({prop, value}) =>
            el.style[prop] = value)

        index >= globalThis.copied_styles.length - 1
          ? index = 0
          : index++
      })
    }
  ...
  const getElementSkeleton = (el, tabLevel, hasChildrenWithoutChildren) => {
    ...
    for(const child of el.children) {
      ...
      if(child.children.length) {
        const hasChildrenWithoutChildren = !(Array.from(child.children).some((innerChild: any) => !innerChild.children.length) 
                                              && Array.from(child.children).some((innerChild: any) => innerChild.children.length)
                                          )
  ...
  const createLabel = ({el, id, template}) => {
    if (!labels[id]) {
      const label = document.createElement('visbug-label') as any
  ...
  const createHandle = ({el, id}) => {
    if (!handles[id]) {
      const handle = document.createElement('visbug-handles') as any
