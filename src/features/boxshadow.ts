...
export function BoxShadow({selection}: {selection: () => HTMLElement[]}) {
...
const ensureHasShadow = (el: HTMLElement) => {
...
const parseCurrentShadow = (el: HTMLElement) => getStyle(el, 'boxShadow')?.split(' ')

export function changeBoxShadow(els: HTMLElement[], direction: string[], prop: string) {
  els
    .map(ensureHasShadow)
    ...
    .map(el => ({
      ...
      value:     parseCurrentShadow(el),
    }))
    .map(payload => {
      let updated = [...payload.current!]
      let cur: any     = prop === 'opacity'
