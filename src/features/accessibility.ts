...
import { readability, isReadable, ColorInput } from '@ctrl/tinycolor'
import {
...
export function Accessibility(visbug: any) {
...
export function showTip(target: HTMLElement, e: MouseEvent) {
...
export function positionTip(tip: HTMLElement, e: MouseEvent) {
...
const render = (el: HTMLElement, tip: HTMLElement & { meta?: any } = document.createElement('visbug-ally')) => {
...
const determineColorContrast = (el: HTMLElement) => {
  ...
  const foreground = el instanceof SVGElement
    ? (getStyle(el, 'fill') || getStyle(el, 'stroke')) as ColorInput
    : getStyle(el, 'color') as ColorInput

  const textSize  = getWCAG2TextSize(el) === 'small'
    ? 'AA'
    : 'AA+'

  let background  = getComputedBackgroundColor(el) as ColorInput
  let pass_icon = `
  ...
  const [ aa_contrast, aaa_contrast ] = [
    isReadable(background, foreground, { level: "AA", size: getWCAG2TextSize(el) }),
    isReadable(background, foreground, { level: "AAA", size: getWCAG2TextSize(el) })
  ]
...
const mouse_quadrant = (e: MouseEvent) => ({
  north: e.clientY > window.innerHeight / 2,
  west:  e.clientX > window.innerWidth / 2
})

const tip_position = (node: HTMLElement, e: MouseEvent, north: boolean, west: boolean) => ({
  top: `${north
...
const handleBlur = ({target}: {target: HTMLElement}) => {
  if (!target.hasAttribute('data-allytip') && state.tips.has(target))
    wipe(state.tips.get(target)!)
}

const wipe = ({tip, e:{target}}: AllyTip) => {
  tip.remove()
  unobserve({tip, target})
  state.tips.delete(target)
}

const togglePinned = (els: HTMLElement[]) => {
  ...
  els
    ...
    .forEach(el => {
      ...
      state.active.tip && state.tips.set(el, {
        tip: state.active.tip,
        e: {target:el},
      })
      clearActive()
...
const observe = ({tip, target}: {tip: HTMLElement, target: HTMLElement}) => {
  $(target).on('DOMNodeRemoved', handleBlur)
}

const unobserve = ({tip, target}: {tip: HTMLElement, target: HTMLElement}) => {
