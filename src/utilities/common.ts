...
export const deepElementFromPoint = (x: number, y: number) => {
  ...
  const crawlShadows = (node: Element | null) => {
    if (node?.shadowRoot) {
...
export const getSide = (direction: string) => {
  let start = direction?.split('+')?.pop()?.replace(/^\w/, (c: string) => c.toUpperCase())
  if (start == 'Up') start = 'Top'
  if (start == 'Down') start = 'Bottom'
  return start
}

export const getNodeIndex = (el: { parentElement: { parentElement: { children: any } } }) => {
  return [...el.parentElement.parentElement.children]
    .indexOf(el.parentElement)
}

export function showEdge(el: { animate: (arg0: { outline: string }[], arg1: number) => any }) {
  return el.animate([
...
export const showHideSelected = (el, duration = 750) => {
  ...
  timeoutMap[nodeKey(el)] = setTimeout((_: any) => {
...
export const isOffBounds = (node: { closest: (arg0: string) => any }) =>
  node.closest && (
...
))((s: any) => document.createDocumentFragment().querySelector(s))
